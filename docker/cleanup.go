...
func CleanupAll(ctx context.Context, logger logrus.FieldLogger) error {
	...
	err := exec.RunCommand(ctx, logger, "docker container stop $(docker container ls -q --filter Name=atlas-) || true", "", nil)
	if err != nil {
		return fmt.Errorf("could not remove containers: %w", err)
	}

	err = exec.RunCommand(ctx, logger, "docker container rm -f -v $(docker container ls -a -q --filter Name=atlas-) || true", "", nil)
	if err != nil {
		return fmt.Errorf("could not remove containers: %w", err)
	}
	_ = bar.Add(1)
	...
	err = exec.RunCommand(ctx, logger, "docker volume rm -f $(docker volume ls -q --filter Name=atlas-) || true", "", nil)
	if err != nil {
	...
	err = exec.RunCommand(ctx, logger, "docker network rm $(docker network ls -q --filter Name=atlas-) || true", "", nil)
	if err != nil {
